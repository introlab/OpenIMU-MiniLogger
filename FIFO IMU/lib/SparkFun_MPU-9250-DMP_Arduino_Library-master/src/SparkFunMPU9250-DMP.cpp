...
inv_error_t MPU9250_DMP::batchUpdateFifo(unsigned char nsamples)
{
	short gyro[3*PACKET_BUFFER_SIZE], accel[3*PACKET_BUFFER_SIZE];
	unsigned long timestamp;
	unsigned short sample_offset;
	unsigned char sensors, more;
	unsigned char isamples;

	if(nsamples > PACKET_BUFFER_SIZE)
		return INV_ERROR;

	if (mpu_batch_read_fifo(gyro, accel, &timestamp, &sensors, nsamples) != INV_SUCCESS)
		return INV_ERROR;

	for(isamples = 0; isamples < nsamples; isamples++) {
		sample_offset = isamples * 3;

		if (sensors & INV_XYZ_ACCEL) {
			axs[isamples] = accel[sample_offset + X_AXIS];
			ays[isamples] = accel[sample_offset + Y_AXIS];
			azs[isamples] = accel[sample_offset + Z_AXIS];
		}
		if (sensors & INV_X_GYRO)
			gxs[isamples] = gyro[sample_offset + X_AXIS];
		if (sensors & INV_Y_GYRO)
			gys[isamples] = gyro[sample_offset + Y_AXIS];
		if (sensors & INV_Z_GYRO)
			gzs[isamples] = gyro[sample_offset + Z_AXIS];
	}
