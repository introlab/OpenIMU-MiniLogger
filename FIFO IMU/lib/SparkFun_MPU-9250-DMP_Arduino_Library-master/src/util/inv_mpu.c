...
int mpu_batch_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
    unsigned char *sensors, unsigned char npackets)
{
    /* Assumes maximum packet size i2c buffer */
    unsigned char data[128];
    unsigned char *packet_ptr;
    unsigned char packet_size = 0;
    unsigned short fifo_count, index, frame_offset = 0;
    unsigned char ipackets;
    ...
    // Calc packet size according to selected sensors
    if (st.chip_cfg.fifo_enable & INV_X_GYRO)
        packet_size += 2;
    if (st.chip_cfg.fifo_enable & INV_Y_GYRO)
        packet_size += 2;
    ...
    // Check is requested packets can be read
    if(packet_size * npackets > 128)
        return -1;
    if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
    fifo_count = (data[0] << 8) | data[1];
    if (fifo_count < packet_size * npackets)
        return -1;
    ...
    // Read data
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, packet_size * npackets, data))
        return -1;

    // Split frames
    for(ipackets = 0; ipackets < npackets; ipackets++) {
        packet_ptr = &data[ipackets * packet_size];
        index = 0;
        frame_offset = ipackets * 3;

        if (st.chip_cfg.fifo_enable & INV_XYZ_ACCEL) {
            accel[frame_offset + 0] = (packet_ptr[index+0] << 8) | packet_ptr[index+1];
            accel[frame_offset + 1] = (packet_ptr[index+2] << 8) | packet_ptr[index+3];
            accel[frame_offset + 2] = (packet_ptr[index+4] << 8) | packet_ptr[index+5];
            index += 6;
        }
        if (st.chip_cfg.fifo_enable & INV_X_GYRO) {
            gyro[frame_offset + 0] = (packet_ptr[index+0] << 8) | packet_ptr[index+1];
            index += 2;
        }
        if (st.chip_cfg.fifo_enable & INV_Y_GYRO) {
            gyro[frame_offset + 1] = (packet_ptr[index+0] << 8) | packet_ptr[index+1];
            index += 2;
        }
        if (st.chip_cfg.fifo_enable & INV_Z_GYRO) {
            gyro[frame_offset + 2] = (packet_ptr[index+0] << 8) | packet_ptr[index+1];
        }
    }
