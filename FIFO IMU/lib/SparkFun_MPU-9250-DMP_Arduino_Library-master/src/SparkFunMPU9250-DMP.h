...
#define PACKET_BUFFER_SIZE 10 // Sample buffer size
...
public:
	int ax, ay, az;
	int gx, gy, gz;
	int axs[PACKET_BUFFER_SIZE], ays[PACKET_BUFFER_SIZE], azs[PACKET_BUFFER_SIZE];
	int gxs[PACKET_BUFFER_SIZE], gys[PACKET_BUFFER_SIZE], gzs[PACKET_BUFFER_SIZE];
	int mx, my, mz;
	long qw, qx, qy, qz;
	long temperature;
	unsigned long time;
	...
	// batchUpdateFifo -- Reads from the top of the FIFO, and stores the new data
	// in ax, ay, az, gx, gy, or gz vectors
	// Output: INV_SUCCESS (0) on success, otherwise error
	inv_error_t batchUpdateFifo(unsigned char nsamples);
